...
//			       System.out.println("Operation done successfully");
	...
	public void InsertAnalyticsData(User userEntity, Analytics analyticsData){
		session.beginTransaction();
		session.save(analyticsData);
	...
	public void updateAnalyticsData(Analytics analyticsData){
		session.beginTransaction();
		session.merge(analyticsData);
		session.getTransaction().commit();
		session.close();
		session.getSessionFactory().close();
	}
	
	public Analytics getAnalyticsData(int analyticsID){
		Analytics userData = new Analytics();
		session = sessionFactory.openSession();
		session.beginTransaction();
		userData = (Analytics)session.get(Analytics.class, analyticsID);
		session.close();
		session.getSessionFactory().close();
		return userData;
}
